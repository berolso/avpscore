...
def remove_user(phone):
    ...
    session.pop("phone")
    flash(
        f"{user.full_name} - {user.phone} has been removed from the system. To continue to recieve texts you will have to create an account again",
        "danger",
    )
...
def status():
    ...
    return render_template(
        "/status.html", jobs=jobs, event=event, send_status=send_status, matches=matches
    )
...
def reset_token(token):
    ...
    if user is None:
        flash(
            "Using is an invalid or expired token. Request password reset again",
            "warning",
        )
