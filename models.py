...
class SendStatus(db.Model):
    ...
    def find_and_add_matches(cls):
        ...
        new_adds = Match.query.filter(
            (Match.match_sent == None)
            & (Match.match_state != "F")
            & (Match.match_state != "C")
        ).all()
    ...
    def finished_matches(cls):
        ...
        finished_matches = (
            ...
            .join(cls)
            .filter((cls.has_sent == False) & (Match.winner != None))
            .all()
        )
...
class User(db.Model):
    ...
    phone = db.Column(
        ...
        nullable=False,
        unique=True,
        info={"label": "Phone Number (must begin with 1)"},
    )
    ...
    email = db.Column(
        db.String, nullable=False, unique=True, info={"validators": Email()}
    )
...
class EventTracker(db.Model):
    ...
    def set_event_status(self, obj):
        ...
        finished_finals = [
            m["Bracket"]
            for m in obj
            if m["Bracket"] == "Finals" and m["MatchState"] == "F"
        ]
