...
def welcome_message(message):
    bot.send_message(
        message.chat.id,
        f"""Thanks for trying the AVP score bot. 
...
def get_updates(message):
    ...
    try:
        ...
        if res == "success":
            bot.send_message(
                id,
                "You are signed up for score updates. 👉 /stop to stop receiving updates",
            )
            bot.send_message(
                TELEGRAM_USER_LOG,
                f"ADDED {message.chat.first_name} {message.chat.last_name} id:{message.chat.id}",
            )
        else:
...
def stop_updates(message):
    ...
    try:
        ...
        bot.send_message(
            TELEGRAM_USER_LOG,
            f"REMOVED {message.chat.first_name} {message.chat.last_name} id:{message.chat.id}",
        )
    except:
...
def bracket(message):
    ...
    bracket_btn.add(
        telebot.types.InlineKeyboardButton(
            "live bracket", url="https://avp.com/brackets/"
        )
    )
...
@bot.message_handler(
    ...
    and message.reply_to_message.text == feedback_question
)
def received_feedback(message):
    ...
    bot.send_message(
        TELEGRAM_FEEDBACK,
        f"""{message.chat.first_name} {message.chat.last_name} id:{message.chat.id} -
  {message.text}""",
    )
...
def echo(message):
    bot.send_message(
        message.chat.id, f"OK {message.chat.first_name}.. If you need 👉/help."
    )
    bot.send_message(
        MY_TELEGRAM,
        f"{message.chat.first_name} {message.chat.last_name} is {message.chat.id} - they said: {message.text}",
    )
